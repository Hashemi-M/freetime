...
def run(config):
    ...
    for initialization in INITIALIZATIONS:
        ...
        for run in range(config.num_runs):
            ...
            results_no_reward[initialization].append(rewards)
            Q_tables[initialization] = Q
            F_tables[initialization] = F
        
    if config.freetime.show_trajectory: 
        for initialization, Q in Q_tables.items():
            make_trajectory_map(Q, env, title=f'Init {initialization} freetime no_reward trajectory', 
                                num_plots=config.trajectory_maps.num_plots)
    
    if config.freetime.show_rewards: 
        plt.figure()
        for title, rewards in results_no_reward.items():
            plot_errorbars(rewards, label=title)
            plt.legend()
            plt.title('freetime no_reward')
        
    if config.freetime.show_q:
        for title, Q in Q_tables.items():
            plot_Q(Q, f'{title} init q-table freetime no_reward', config.q_plots.vmin, config.q_plots.vmax)
        
    if config.freetime.show_f:
        for action in config.freetime.show_f_actions:
            for title, F in F_tables.items():
                plot_F(F, f'{title} init f-table freetime no_reward action {action}', config.f_plots.vmin, config.f_plots.vmax, 
                       action)
    ...
    if config.plot_freetime_vs_baseline_same_table:
        for initialization in INITIALIZATIONS:
            ...
            plot_errorbars(results_no_reward[initialization], label='no_reward')
